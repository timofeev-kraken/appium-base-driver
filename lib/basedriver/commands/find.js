...
/**
 * Find an element using a custom plugin specified by the customFindModule cap.
 *
...
commands.findByCustom = async function (selector, multiple) {
  if (!this.opts.customFindModule) {
    throw new Error("Finding an element using a plugin is currently an " +
                    ...
                    "and set the capability 'customFindModule' to the name of " +
                    "that module so that it can be required.");
  }
  ...
  try {
    ...
    throw new Error(`Could not load your custom find module ` +
                    `'${this.opts.customFindModule}'. Original error: ${err}`);
  }
  ...
  const condition = async () => {
    ...
    // optimizations
    elements = await finder.find(this, customFinderLog, selector, multiple);
